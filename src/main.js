...
let state={user:null,hybrids:[],farms:[],fields:[],stations:[],priors:{},observed:[],phenObserved:[],phenProfiles:{CORE:{heat_weight:.50,drought_weight:.30,excess_weight:.15,cold_weight:.05,band_weight:.60},SHOULDER:{heat_weight:.40,drought_weight:.30,excess_weight:.20,cold_weight:.10,band_weight:.40}},model:{...DEFAULT_MODEL},history:[],tab:'dashboard',last:null,charts:[]};
...
function dashboardView(){
  ...
  return `<div class="grid"><div class="card span3"><div class="muted">Campos</div><div class="kpi">${farms}</div></div><div class="card span3"><div class="muted">Lotes</div><div class="kpi">${fields}</div></div><div class="card span3"><div class="muted">Simulaciones</div><div class="kpi">${state.history.length}</div></div><div class="card span3"><div class="muted">Motor climático</div><div class="kpi">${obs?'HISTÓRICO':'PRIORS'} <small>${obs?'NASA POWER':'fallback'}</small></div></div>
...
async function runSimulation(e){
  ...
  const bundle=riskBundleAtR1(fl.date,phase,stationId),r=bundle.values,scorePack=scoreWithPhenology(fl.date,phase,stationId,r),score=scorePack.score;
  const alts=[];
  for(let delta=-20;delta<=20;delta+=5){
    ...
    const sp=scoreWithPhenology(ff.date,phase,stationId,bb.values);alts.push({date:sd,score:sp.score,r1:ff.date,source:sp.phen?'phenology_historical':bb.source});
  }
  ...
  const density=Number($('#density').value)||null,water=Number($('#water').value)||null,environment=$('#environment').value||null;
  ...
  let recommendation=makeRecommendation(r,score,best,h,density,environment,bundle.source);
  ...
  state.last={h,phen,sow,fl,cs,ce,r,score,phenScore:scorePack.phen,alts,best,phase,field,fieldName,density,water,environment,recommendation,source:scorePack.phen?'phenology_historical':bundle.source,riskDetails:bundle.details,riskFortnight:bundle.key,lat:hasGeo?lat:null,lon:hasGeo?lon:null,accuracy,locationSource,climateCellLat,climateCellLon,station};
...
function bindFields(){
  ...
  $('#fieldForm').addEventListener('submit',async e=>{e.preventDefault();const lat=$('#fieldLat').value?Number($('#fieldLat').value):null,lon=$('#fieldLon').value?Number($('#fieldLon').value):null;if((lat!=null||lon!=null)&&!validCoord(lat,lon))return alert('Revisá latitud y longitud del lote.');const payload={farm_id:$('#fieldFarm').value,name:$('#newFieldName').value.trim(),area_ha:$('#fieldArea').value?Number($('#fieldArea').value):null,soil_depth:$('#soilDepth').value||null,environment:$('#fieldEnv').value||null,latitude:lat,longitude:lon,geo_accuracy_m:Number($('#fieldGeo').dataset.accuracy)||null,location_source:Number($('#fieldGeo').dataset.accuracy)?'device':(lat!=null?'manual_or_farm':'none')};const {error}=await sb.from('fields').insert(payload);if(error)return alert(error.message);await loadUserData();render();});
