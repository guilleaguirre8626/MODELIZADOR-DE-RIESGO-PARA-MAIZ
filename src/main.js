...
function authCard(inner){return `<div class="login"><div class="card"><div class="brand"><h1>Modelizador de Riesgo para Maíz <span class="version-badge">v2.9.6</span></h1><p>Riesgo agroclimático y decisiones de siembra.</p></div>${inner}</div></div>`}
...
function hydricHtml(h){if(!h)return `<div class="note warning"><b>Balance hídrico no disponible</b><br>Falta cargar la serie diaria con ETo en Supabase. El score sigue usando el proxy histórico de precipitación.</div>`;if(h.error)return `<div class="note warning"><b>Balance hídrico no disponible</b><br>${esc(h.error)}</div>`;if(!h.n)return `<div class="note warning"><b>Sin campañas hídricas comparables</b><br>No hubo datos diarios suficientes para esta combinación.</div>`;return `<div class="note hydric-summary"><b>💧 Estado hídrico histórico estimado a R1</b><br><div class="profile-grid hydric-five" style="margin-top:10px"><div class="mini">Riesgo de estrés en R1<b>${h.riskStress.toFixed(1)}%</b></div><div class="mini">≥3 días con estrés<b>${h.riskStress3.toFixed(1)}%</b></div><div class="mini">Lluvia mediana siembra→R1<b>${h.medianRain.toFixed(0)} mm</b></div><div class="mini">ETc potencial mediana<b>${h.medianETc.toFixed(0)} mm</b></div><div class="mini">Campañas comparables<b>${h.n}</b></div></div><span class="muted">Modelo: balance diario; Kc 0,40→1,15 según avance térmico; ETo Hargreaves; lluvia efectiva 90%; umbral de estrés FAO ajustado por ETc.</span></div>`}
...
function hydricRiskScore(h){
  if(!h || h.error || !h.n) return null;
  const stress=Number(h.riskStress);
  const severe=Number(h.riskSevere);
  const stress3=Number(h.riskStress3);
  if(!Number.isFinite(stress) || !Number.isFinite(severe) || !Number.isFinite(stress3)) return null;
  return Math.max(0,Math.min(100,stress*0.70+severe*0.20+stress3*0.10));
}
...
function phenologyVisualHtml(x){
  const wanted=new Set(['VE','V6','VT','R1','R3','R6']);
  const stages=phenologyStagesForResult(x).filter(s=>wanted.has(s.code));
  const cards=stages.map(s=>{
    const tone=s.code==='R1'?'r1':(s.code==='VT'||s.code==='R3'?'warn':'ok');
    return `<div class="phen2-card phen2-${tone}">
      <div class="phen2-code">${s.code}</div>
      <div class="phen2-date">${fmt(s.date)}</div>
      ${s.code==='R1'?'<div class="phen2-critical">PERÍODO CRÍTICO</div>':''}
      <div class="phen2-line"><span></span></div>
      <div class="phen2-metric"><span>🔥 Calor</span><b>${Number(s.heat||0).toFixed(0)}%</b></div>
      <div class="phen2-metric"><span>💧 Hídrico</span><b>${Number(s.hydric||0).toFixed(0)}%</b></div>
    </div>`;
  }).join('');
  return `<div id="phenologyVisual" class="phenology-panel phenology-v2-card"><div class="phen2-title">EVOLUCIÓN FENOLÓGICA</div><div class="phen2-subtitle">Etapas clave y riesgo esperado</div><div class="phen2-flow">${cards}</div><div class="phen2-legend"><span>🔥 Ola de calor ≥3 días ≥35 °C</span><span>💧 Riesgo hídrico (déficit + persistencia de estrés)</span></div><p class="muted phen2-foot">Probabilidades históricas calculadas por la quincena estimada de cada etapa. R1 utiliza el balance hídrico histórico diario cuando está disponible.</p></div>`;
}

function renderPhenologyVisual(x){
  const mount=$('#phenologyMount');
  if(!mount)return;
  try{
    mount.innerHTML=phenologyVisualHtml(x);
  }catch(err){
    console.error('Error renderizando fenología',err);
    mount.innerHTML=`<div class="note warning"><b>Visualización fenológica no disponible</b><br>${esc(err?.message||'No se pudieron calcular las etapas.')}</div>`;
  }
}

function renderResult(){const x=state.last;const [label,klass]=riskClass(x.score);const engine=x.source==='phenology_historical'?'HISTÓRICO + SCORE FENOLÓGICO':x.source==='historical'?'HISTÓRICO':x.source==='mixed'?'HISTÓRICO + FALLBACK':'PRIORS';$('#result').innerHTML=`<div class="section-title"><h2>Resultado <span class="version-badge">v2.9.6</span></h2><span class="badge ${klass}">${label}</span></div><div class="kpi">${x.score}/100 <small>score combinado</small></div><p><b>R1 estimada:</b> ${fmt(x.fl.date)} · <b>Período crítico:</b> ${fmt(x.cs)}–${fmt(x.ce)}</p><p class="muted">Fenología usada: ${x.phen.gdu} GDU base ${x.phen.base}°C (${x.phen.source}). · Quincena Data Risk: ${x.riskFortnight}</p>${x.lat!=null?`<p class="muted"><b>Georreferencia:</b> ${x.lat.toFixed(5)}, ${x.lon.toFixed(5)}${x.station?` · referencia ${esc(x.station.name||'')}`:''}</p>`:''}<div class="note ${x.source==='priors'?'warning':''}"><b>Motor climático: ${engine}</b><br>${x.source==='phenology_historical'?'NASA POWER + ENSO con subventanas fenológicas. El calor pesa 50% dentro de R1 ±7 días y 40% entre 8–15 días; los pesos se leen desde Supabase.':x.source==='historical'?'NASA POWER diario + clasificación histórica ENSO. La probabilidad representa campañas con el evento dentro de la ventana crítica asociada a esa quincena de R1.':x.source==='mixed'?'Hay datos históricos parciales; los faltantes se completaron con PRIORS.':'No se encontró una matriz histórica coincidente y se usaron PRIORS como respaldo.'}</div>${phenScoreHtml(x)}${hydricHtml(x.hydric)}<div class="risk-details">${detailLine('🔥 Ola de calor ≥3 días ≥35 °C','heat',x)}${detailLine('❄️ Frío ≤4 °C','cold',x)}${detailLine('💧 Riesgo hídrico a R1','drought',x)}${detailLine('🌧️ Exceso >180 mm / 31 días*','excess',x)}</div><p class="muted">* Si el balance hídrico histórico está disponible, el componente de déficit del score usa la probabilidad de llegar a R1 con menos de 45% del AU máximo. Si no, usa el proxy histórico de precipitación.</p><div class="note"><b>Recomendación</b><br>${esc(x.recommendation)}</div><div class="actions"><button id="pdfBtn" class="primary">Descargar PDF</button><button id="hybridBtn" class="secondary">Ver ficha del híbrido</button></div>`;const wide=$('#phenologyWide');if(wide){wide.innerHTML=phenologyVisualHtml(x);wide.style.display='block';}$('#pdfBtn').addEventListener('click',downloadPdf);$('#hybridBtn').addEventListener('click',()=>{state.tab='hybrids';render();setTimeout(()=>{const el=document.querySelector(`[data-hybrid-card="${x.h.id}"]`);el?.scrollIntoView({behavior:'smooth',block:'start'})},100)});}
...
function historyView(){return `<div class="card"><div class="section-title"><h2>Historial de simulaciones</h2><span class="muted">${state.history.length} registros</span></div>${state.history.length?`<div class="history-toolbar"><label class="history-select-all"><input id="historySelectAll" type="checkbox"> <span>Seleccionar todos</span></label><div class="history-bulk-actions"><span id="historySelectedCount" class="muted">0 seleccionados</span><button id="historyPdfSelected" class="primary" disabled>Descargar seleccionados en PDF</button><button id="historyDeleteSelected" class="danger" disabled>Eliminar seleccionados</button></div></div><div class="table-wrap"><table class="table"><thead><tr><th class="select-col">✓</th><th>Fecha</th><th>Campo/Lote</th><th>Híbrido</th><th>Siembra</th><th>R1</th><th>ENSO</th><th>Score</th><th></th></tr></thead><tbody>${state.history.map(x=>`<tr><td class="select-col"><input type="checkbox" data-select-sim="${x.id}" aria-label="Seleccionar simulación"></td><td>${new Date(x.created_at).toLocaleString('es-AR')}</td><td>${esc(x.field_name||'')}</td><td>${esc(x.hybrids?.name||x.hybrid_snapshot||'')} ${esc(x.hybrids?.biotechnology||'')}</td><td>${esc(x.sowing_date||'')}</td><td>${esc(x.flowering_date||'')}</td><td>${esc(x.enso_phase||'')}</td><td><span class="badge ${riskClass(Number(x.risk_score||0))[1]}">${Math.round(x.risk_score||0)}/100</span></td><td><button class="danger" data-delete-sim="${x.id}">Eliminar</button></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">Todavía no hay simulaciones.</div>'}</div>`}
function selectedHistoryItems(){const ids=[...document.querySelectorAll('[data-select-sim]:checked')].map(x=>x.dataset.selectSim);return state.history.filter(x=>ids.includes(String(x.id)))}
function updateHistorySelection(){const boxes=[...document.querySelectorAll('[data-select-sim]')];const checked=boxes.filter(x=>x.checked);const count=$('#historySelectedCount');const pdfBtn=$('#historyPdfSelected');const delBtn=$('#historyDeleteSelected');const all=$('#historySelectAll');if(count)count.textContent=`${checked.length} seleccionado${checked.length===1?'':'s'}`;if(pdfBtn)pdfBtn.disabled=checked.length===0;if(delBtn)delBtn.disabled=checked.length===0;if(all){all.indeterminate=checked.length>0&&checked.length<boxes.length;all.checked=boxes.length>0&&checked.length===boxes.length}}
function historyHybridLabel(x){const name=x.hybrids?.name||x.hybrid_snapshot||'Sin híbrido';const bio=x.hybrids?.biotechnology||'';return `${name}${bio?' — '+bio:''}`}
function addPdfField(d,label,value,y){d.setFont(undefined,'bold');d.text(label+':',14,y);d.setFont(undefined,'normal');const lines=d.splitTextToSize(String(value??'s/d'),135);d.text(lines,60,y);return y+Math.max(7,lines.length*5)}
async function deleteSelectedHistory(){const items=selectedHistoryItems();if(!items.length)return alert('Seleccioná al menos una simulación.');const n=items.length;if(!confirm(`Vas a eliminar ${n} simulación${n===1?'':'es'} del historial. Esta acción no se puede deshacer. ¿Continuar?`))return;const ids=items.map(x=>x.id);const {error}=await sb.from('simulations').delete().in('id',ids);if(error)return alert('No se pudieron eliminar las simulaciones seleccionadas: '+error.message);await loadUserData();render();}
function downloadSelectedHistoryPdf(){const items=selectedHistoryItems();if(!items.length)return alert('Seleccioná al menos una simulación.');if(!jsPDF)return alert('El generador de PDF no está disponible.');const d=new jsPDF();items.forEach((x,i)=>{if(i>0)d.addPage();d.setFontSize(18);d.setFont(undefined,'bold');d.text('Modelizador de Riesgo para Maíz',14,18);d.setFontSize(10);d.setFont(undefined,'normal');d.text(`Historial seleccionado · Informe ${i+1} de ${items.length}`,14,25);let y=37;y=addPdfField(d,'Fecha del análisis',new Date(x.created_at).toLocaleString('es-AR'),y);y=addPdfField(d,'Campo/Lote',x.field_name||'s/d',y);y=addPdfField(d,'Híbrido',historyHybridLabel(x),y);y=addPdfField(d,'Fecha de siembra',x.sowing_date||'s/d',y);y=addPdfField(d,'R1 estimada',x.flowering_date||'s/d',y);y=addPdfField(d,'Período crítico',`${x.critical_start||'s/d'} a ${x.critical_end||'s/d'}`,y);y=addPdfField(d,'ENSO',x.enso_phase||'s/d',y);y=addPdfField(d,'Score combinado',`${Math.round(Number(x.risk_score||0))}/100 — ${riskClass(Number(x.risk_score||0))[0]}`,y);if(x.risk_heat!=null)y=addPdfField(d,'Riesgo de ola de calor',`${Number(x.risk_heat).toFixed(1)}%`,y);if(x.risk_cold!=null)y=addPdfField(d,'Riesgo de frío',`${Number(x.risk_cold).toFixed(1)}%`,y);if(x.risk_excess!=null)y=addPdfField(d,'Riesgo de exceso hídrico',`${Number(x.risk_excess).toFixed(1)}%`,y);if(x.hydric_stress_probability!=null)y=addPdfField(d,'Riesgo de estrés hídrico en R1',`${Number(x.hydric_stress_probability).toFixed(1)}%`,y);if(x.hydric_stress3_probability!=null)y=addPdfField(d,'≥3 días con estrés',`${Number(x.hydric_stress3_probability).toFixed(1)}%`,y);if(x.water_initial_mm!=null)y=addPdfField(d,'Agua útil inicial',`${Number(x.water_initial_mm).toFixed(0)} mm`,y);if(x.water_capacity_mm!=null)y=addPdfField(d,'AU máxima del perfil',`${Number(x.water_capacity_mm).toFixed(0)} mm`,y);if(x.water_r1_median_mm!=null)y=addPdfField(d,'AU mediana en R1',`${Number(x.water_r1_median_mm).toFixed(0)} mm${x.water_r1_median_pct!=null?` (${Number(x.water_r1_median_pct).toFixed(0)}%)`:''}`,y);if(x.latitude!=null&&x.longitude!=null)y=addPdfField(d,'Georreferencia',`${Number(x.latitude).toFixed(5)}, ${Number(x.longitude).toFixed(5)}`,y);if(y>235){d.addPage();y=20}d.setFont(undefined,'bold');d.text('Recomendación',14,y+5);d.setFont(undefined,'normal');d.text(d.splitTextToSize(x.recommendation||'Sin recomendación guardada.',180),14,y+12);d.setFontSize(8);d.text(`Registro ${i+1}/${items.length} · ${x.model_version||'modelo histórico'}`,14,286)});const stamp=new Date().toISOString().slice(0,10);d.save(`Historial_Modelizador_Maiz_${stamp}_${items.length}_simulaciones.pdf`)}
function bindHistory(){document.querySelectorAll('[data-delete-sim]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('¿Eliminar esta simulación?'))return;const {error}=await sb.from('simulations').delete().eq('id',b.dataset.deleteSim);if(error)return alert(error.message);await loadUserData();render();}));document.querySelectorAll('[data-select-sim]').forEach(b=>b.addEventListener('change',updateHistorySelection));$('#historySelectAll')?.addEventListener('change',e=>{document.querySelectorAll('[data-select-sim]').forEach(b=>b.checked=e.target.checked);updateHistorySelection()});$('#historyPdfSelected')?.addEventListener('click',downloadSelectedHistoryPdf);$('#historyDeleteSelected')?.addEventListener('click',deleteSelectedHistory);updateHistorySelection();}
