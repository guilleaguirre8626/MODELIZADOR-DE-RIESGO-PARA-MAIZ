...
function scorePhenologyWithHydric(pack,r){if(!pack.phen)return scoreRisk(r);const bands=pack.phen.bands||pack.phen;const core=bands.CORE,shoulder=bands.SHOULDER;const cp=state.phenProfiles.CORE||{},sp=state.phenProfiles.SHOULDER||{};const coreScore=core?core.values.heat*Number(cp.heat_weight||.5)+r.drought*Number(cp.drought_weight||.3)+core.values.excess*Number(cp.excess_weight||.15)+core.values.cold*Number(cp.cold_weight||.05):0;const shoulderScore=shoulder?shoulder.values.heat*Number(sp.heat_weight||.4)+r.drought*Number(sp.drought_weight||.3)+shoulder.values.excess*Number(sp.excess_weight||.2)+shoulder.values.cold*Number(sp.cold_weight||.1):0;return Math.round(coreScore*Number(cp.band_weight||.6)+shoulderScore*Number(sp.band_weight||.4));}
...
async function runSimulation(e){
  ...
  const scorePack=scoreWithPhenology(fl.date,phase,stationId,r);const score=scorePack.phen?scorePhenologyWithHydric(scorePack,r):scoreRisk(r);
  const alts=[];
  ...
  state.last={h,phen,sow,fl,cs,ce,r,score,phenScore:scorePack.phen,alts,best,phase,field,fieldName,density,water,waterCapacity,hydric,environment,recommendation,source:scorePack.phen?'phenology_historical':bundle.source,riskDetails:bundle.details,riskFortnight:bundle.key,lat:hasGeo?lat:null,lon:hasGeo?lon:null,accuracy,locationSource,climateCellLat,climateCellLon,station};
