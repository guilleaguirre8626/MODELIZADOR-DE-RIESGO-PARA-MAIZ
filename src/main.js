...
function authCard(inner){return `<div class="login"><div class="card"><div class="brand"><h1>Modelizador de Riesgo para Maíz <span class="version-badge">v2.9.0</span></h1><p>Riesgo agroclimático y decisiones de siembra.</p></div>${inner}</div></div>`}
...
function phenologyVisualHtml(x){
  ...
  const cards=stages.map(s=>{
    const thermal=Math.max(Number(s.heat||0),Number(s.cold||0));
    const thermalName=Number(s.heat||0)>=Number(s.cold||0)?'Calor':'Frío';
    const isCritical=s.code==='VT'||s.code==='R1';
    return `<div class="phen-stage-card ${s.code==='R1'?'phen-stage-r1':''} ${isCritical?'phen-stage-critical':''}">
      <div class="phen-stage-top"><span class="phen-stage-icon">${s.icon}</span><div><b>${s.code}</b><small>${esc(s.label)}</small></div></div>
      <div class="phen-stage-date">${fmt(s.date)}</div>
      <div class="phen-stage-metric"><span>🔥 Riesgo térmico</span>${stageRiskCell(thermal,`${thermalName}: ${thermal.toFixed(0)}% · ${s.key}`)}</div>
      <div class="phen-stage-metric"><span>💧 Riesgo hídrico</span>${stageRiskCell(s.hydric,`${s.code} · ${s.key} · ${s.source}`)}</div>
    </div>`;
  }).join('<div class="phen-arrow">→</div>');
  return `<div id="phenologyVisual" class="phenology-panel phenology-compact"><div class="phenology-kicker">🌽 EVOLUCIÓN FENOLÓGICA</div><div class="section-title"><div><h3>Etapas clave y riesgo esperado</h3><p class="muted">Vista simplificada para decisión. VT–R1 se destaca por ser el período de mayor sensibilidad.</p></div><span class="badge info">VT–R1 crítico</span></div><div class="phen-compact-flow">${cards}</div><p class="muted phen-foot">Riesgo térmico = mayor probabilidad entre calor y frío en la quincena estimada. En R1, el riesgo hídrico usa el balance histórico diario cuando está disponible; en las demás etapas se muestra el proxy histórico de su quincena.</p></div>`;
}
...
function updateHistorySelection(){const boxes=[...document.querySelectorAll('[data-select-sim]')];const checked=boxes.filter(x=>x.checked);const count=$('#historySelectedCount');const btn=$('#historyPdfSelected');const all=$('#historySelectAll');if(count)count.textContent=`${checked.length} seleccionado${checked.length===1?'':'s'}`;if(btn)btn.disabled=checked.length===0;if(all){all.indeterminate=checked.length>0&&checked.length<boxes.length;all.checked=boxes.length>0&&checked.length===boxes.length}}
