...
function scorePhenologyWithHydric(pack,r){if(!pack.phen)return scoreRisk(r);const core=pack.phen.CORE,shoulder=pack.phen.SHOULDER;const cp=state.phenProfiles.CORE||{},sp=state.phenProfiles.SHOULDER||{};const coreScore=core?core.values.heat*Number(cp.heat_weight||.5)+r.drought*Number(cp.drought_weight||.3)+core.values.excess*Number(cp.excess_weight||.15)+core.values.cold*Number(cp.cold_weight||.05):0;const shoulderScore=shoulder?shoulder.values.heat*Number(sp.heat_weight||.4)+r.drought*Number(sp.drought_weight||.3)+shoulder.values.excess*Number(sp.excess_weight||.2)+shoulder.values.cold*Number(sp.cold_weight||.1):0;return Math.round(coreScore*Number(cp.band_weight||.6)+shoulderScore*Number(sp.band_weight||.4));}
